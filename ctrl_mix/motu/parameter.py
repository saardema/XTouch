...
if TYPE_CHECKING:
    from motu.mixer import MotuMixerChannel
...
class MotuParameter(Parameter, ABC):
    ...
    def create(cls, channel: MotuMixerChannel, path: str, mix_state: dict[str, Any]):
        cfg_type, initial, min_val, max_val, unit = cls.read_config(path, mix_state)
        ...
        if issubclass(p_cls, FloatParam):
            if p_name in ("fader", "send"):
                instance = VolumeParam(channel, path, initial, min_val, max_val)
            else:
                instance = FloatParam(channel, path, initial, min_val, max_val)
        else:
            instance = p_cls(channel, path, initial)
    ...
    def read_config(path: str, mix: dict[str, Any]):
        ...
        return cfg_type, initial, min_val, max_val, unit


class FloatParam(MotuParameter):
    value_type = float

    def __init__(self, channel: MotuMixerChannel, path: str, initial: TParam, min_val: TParam | None, max_val: TParam | None) -> None:
        super().__init__(channel, path)
        self._value = float(initial)
        self._normalized = float(initial)
    ...
    #     new_value = self._before_set_filter(new_value)
    #     self._value = new_value
    #     self._normalized = new_value
...
class VolumeParam(FloatParam):
    ...
    # @value.setter
    # def value(self, new_value: float):
    #     self._value = self._before_set_filter(new_value)
    #     self._normalized = gain_to_norm(self._value)
    ...
    # @normalized.setter
    # def normalized(self, new_value: float):
    #     self._normalized = new_value
    #     self._value = norm_to_gain(new_value)
...
class BoolParam(MotuParameter):
    ...
    def __init__(self, channel: MotuMixerChannel, path: str, initial: TParam) -> None:
        super().__init__(channel, path)
        self._value = float(initial)
        self._normalized = bool(initial)
    ...
    # def value(self, new_value: float):
    #     self._value = new_value
    #     self._normalized = new_value > 0.5
    ...
    # @normalized.setter
    # def normalized(self, new_value: bool):
    #     self._normalized = new_value
    #     self._value = 1.0 if new_value else 0.0
